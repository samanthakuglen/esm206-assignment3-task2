...
# Notes: create a new object of starwars data separating the hair_color column into two columns hair_1 and hair_2. Each entry is separated by a comma, so indicate that as the separation to split it up
...
# Notes: create new object from starwars and separate the name column into 'name_first' and 'name_additional'. These entries are separated by a space " ". extra = "merge" splits everything else into the second column if there's any other extra spaces, don't remove the original column
...
# Notes: creates a new data object from mpg, unites the existing columns manufacture and model into a new column named make_model with a HEY! in between make and model
...
# Notes: ymd converts any type of inputted date to year-month-day format
...
# December 17th, 1997 into YYYY-MM-DD format
ymd("1997-12-17")
...
# Notes: create vector of dates in month-day-year format, convert to dates, return. Class is date. Have to use `mdy because of the order of the dates
...
# Notes: use ymd as an argument to mutate to create add new column `date_new` with converted date. 
...
gasol_hookshots <- lakers %>% 
  filter(player %in% c("Pau Gasol"), type == "hook") %>% 
  mutate(date_new = ymd(date)) %>% 
  mutate(month_new = month(date_new)) %>% 
  mutate(year_new = year(date_new))
...
gasol_monthly_hookshots <- gasol_hookshots %>% 
  ...
  group_by(month_new, year_new) %>% 
  summarize(total_hook_shots = sum(points))
```
...
# Notes: use mutate to add new season columns when there is a certain number for the month
